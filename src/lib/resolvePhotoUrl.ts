/**
 * Mirrors API `mediaUrl` so the storefront can turn public_ids or legacy `/choma/...`
 * paths into Cloudinary delivery URLs when `VITE_CLOUDINARY_CLOUD_NAME` is set.
 *
 * Full `https://...` values pass through (typical when the API already resolved them).
 */
